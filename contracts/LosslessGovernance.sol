...
contract LosslessGovernance is ILssGovernance, Initializable, AccessControlUpgradeable, PausableUpgradeable {
    ...
    function refundContractAddressForInvalidVotes(uint256 _reportId) override public {
        //1. get false report
        (, bool valid) = _getCommitteeMajorityReachedResult(_reportId);
        require(!valid, "Cannot refund a valid report");
        //2. call re-imburse
        (,address reported,address secondReported,,,,) = losslessReporting.getReportInfo(_reportId);
        if(secondReported != address(0)){
            require(isContract(reported),"Reported account is not a Contract");
            require(isContract(secondReported),"Second Reported account is not a Contract");
            uint amount1 = losslessStaking.getStakedAmount(_reportId, reported);
            uint amount2 = losslessStaking.getStakedAmount(_reportId, secondReported);
            losslessReporting.retrieveCompensation(reported, amount1);
            losslessReporting.retrieveCompensation(secondReported, amount2);
            emit refundDoubleContract(reported, secondReported, amount1, amount2);
        } else{
            require(isContract(reported),"Reported account is not a Contract");
            uint amount1 = losslessStaking.getStakedAmount(_reportId, reported);            
            losslessReporting.retrieveCompensation(reported, amount1);
            emit refundSingleContract(reported, amount1);
