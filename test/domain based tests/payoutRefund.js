...
describe(scriptName, () => {
  ...
  describe('when everyone votes negatively', () => {
    ...
    it('should let reported Contract address retrieve compensation', async () => {
      await ethers.provider.send('evm_increaseTime', [
        Number(time.duration.minutes(5)),
      ]);
      ...
      await expect(
        env.lssGovernance
          .connect(adr.maliciousActor1)
          .refundContractAddressForInvalidVotes(1),
      )
        .to.emit(env.lssGovernance, 'refundSingleContract')
